	...
	rows, err := tx.Query(`
		WITH candidate AS (
			...
			SET status = 'leased', run_id = $2, lease_owner = $3, lease_token = $4,
			    lease_expires_at = CURRENT_TIMESTAMP + make_interval(secs => $5), updated_at = CURRENT_TIMESTAMP
			FROM candidate c WHERE q.id = c.id
			RETURNING q.id, q.source_id, q.task_type, q.status, q.priority, q.not_before, q.attempts,
			          q.run_id, q.lease_owner, q.lease_token, q.lease_expires_at, q.last_error, q.created_at, q.updated_at, q.completed_at
		)
		...
		ORDER BY priority::BIGINT + FLOOR(EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - created_at)) / 3600)::BIGINT DESC,
		         priority DESC, created_at ASC, id ASC
	`, sanitizeExploreLimit(batchLimit), run.ID, owner, leaseToken, seconds)
	if err != nil {
		return nil, nil, err
	}
