...
func newRLSFixture(t *testing.T) (*rlsFixture, func()) {
	...
	if _, err := privDB.Exec(`INSERT INTO explore_source_observations (provider_id, source_id, external_key) VALUES ($1, $2, 'fixture')`, providerID, sourceID); err != nil {
		t.Fatalf("seed explore source: %v", err)
	}
	if _, err := privDB.Exec(`INSERT INTO explore_articles (source_id, title, url, normalized_url) VALUES ($1, 'fixture article', 'https://example.test/article', 'https://example.test/article')`, sourceID); err != nil {
