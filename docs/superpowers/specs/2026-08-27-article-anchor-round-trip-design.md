...
- Its visible content is only the existing target icon `⌖`; the accessible name and tooltip remain `跳转原文`.
...
- The return anchor has an `href` targeting the recorded summary source ID. Activating it also prevents default fragment navigation and scrolls back to that exact source anchor.
...
Keep the behavior centralized around `SummaryMarkdown` and the existing strict `article-section` parser. Assign a stable per-render ID to each valid summary source anchor. A single active round-trip controller records the source element and target element, renders the target-side return anchor without altering stored Markdown, and cleans up when either side disappears.
...
- Icon-only controls retain visible focus treatment, `aria-label`, and `title` attributes.

## Verification

Add focused frontend tests before implementation for:

- icon-only forward rendering with a strict article `href`;
- forward scrolling, highlight, and unchanged URL/history;
- return anchor rendering as `↩⌖` with a source-anchor `href`;
- exact return scrolling and cleanup;
