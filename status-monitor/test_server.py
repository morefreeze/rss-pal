...
class Node {
  ...
  focus(options = {}) { if (options.preventScroll && this.ownerDocument.throwPreventScroll) throw new Error('preventScroll unsupported'); const prior = this.ownerDocument.activeElement; if (prior && prior !== this) prior.dispatch('blur'); if (!options.preventScroll) { let ancestor = this.parentNode; while (ancestor && ancestor.className !== 'hour-scroller') ancestor = ancestor.parentNode; if (ancestor) ancestor.scrollLeft = 999; } this.ownerDocument.activeElement = this; this.dispatch('focus'); }
